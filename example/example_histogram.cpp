/**
 * @file example_histogram.cpp
 * @brief 演示 vtkPlot2D 概率分布直方图（双分布对比 + 参考线）的用法
 */
...
int main(int argc, char *argv[])
{
    ...
    vtkPlot2D w;
    w.setWindowTitle("概率分布直方图 - Histogram");
    w.resize(800, 600);

    std::srand(static_cast<unsigned>(std::time(nullptr)));

    // ---- 生成正态分布数据 N(0,1)（Box-Muller 变换）----
    ...
    vtkHistogram *histNormal = w.addHistogram(
        normalData, 50, QColor(0, 255, 255, 120), "N(0,1)");
    histNormal->setTitle("概率分布对比");
    histNormal->setXAxisTitle("数值");
    histNormal->setYAxisTitle("频次");

    // ---- 添加均匀分布直方图：50 分箱，红色半透明 ----
    w.addHistogram(uniformData, 50, QColor(255, 80, 80, 120), "U[-3,3]");

    // ---- 添加 x=0 处的红色虚线参考线 ----
    w.addHistogramRefLine(0.0, QColor(255, 0, 0), 2.0, "x=0");
